...
class TransformerDataFit(Problem):
    ...
    def evaluate(self, individual):
        ...
        R = [math.inf, 2.3448, 1.2745, 0.4992, 0.0]

        R1 = individual.vector[0]
        X1 = individual.vector[1]
        Rv = individual.vector[2]
        ...
        for i in range(len(U1)):
            if i == 0:
                Zp = 1.0 / (1.0 / Rv + 1.0 / (1j * Xm))
            else:
                Zp = 1.0 / (1.0 / Rv + 1.0 / (1j * Xm) + 1.0 / (a*a*R[i] + R2 + 1j * X2))
            Ze = R1 + 1j * X1 + Zp
            aZe = abs(Ze)
            fZe = cmath.phase(Ze)
            ...
            if i < len(U1)-1:
                if i == 0:
                    ...
                    U2c = U1[i] * Zp / Ze * a*a*R[i] / (a*a*R[i] + R2 + 1j * X2)
                U2c = abs(U2c) / a
